...
namespace MxNetLib
{
    ...
    public sealed class SGDOptimizer : BaseOptimizer
    {
        ...
        public override void Update(int index, NDArray weight, NDArray grad)
        {
            ...
            {
                nd.SgdUpdate(weight, grad,  this.GetLearningRate(index), this.GetWeightDecay(index));
            }
            else
            {
                inputs[2] = this._States[index].GetHandle();
                nd.SgdMomUpdate(weight, grad, new NDArray(inputs[2]), this.GetLearningRate(index), this.GetWeightDecay(index));
            }
        }
