...
namespace MxNetLib.Optimizers
{
    public class Adam : BaseCls
    {
        ...
        public Adam(float lr = 0.01f, float beta_1 = 0.9f, float beta_2 = 0.999f, float decayRate = 0, bool amsGrad = false,  float epsilon = 1e-7f) : base(lr, "adam")
        {
            ms = new Dictionary<int, NDArray>();
            vs = new Dictionary<int, NDArray>();
            vhats = new Dictionary<int, NDArray>();
            Beta1 = beta_1;
            Beta2 = beta_2;
            DecayRate = decayRate;
            Epsilon = epsilon;
