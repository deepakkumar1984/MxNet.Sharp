...
namespace MxNet.Gluon.RNN
{
    public class BidirectionalCell : HybridRecurrentCell
    {
        ...
        public override (NDArrayOrSymbol[], NDArrayOrSymbol[]) Unroll(int length, NDArrayOrSymbol[] inputs, NDArrayOrSymbol[] begin_state = null, string layout = "NTC", bool? merge_outputs = null, Symbol valid_length = null)
        {
            ...
            var (l_outputs, l_states) = l_cell.Unroll(length, inputs, states.Take(l_cell.StateInfo().Length).ToArray(), layout, merge_outputs, valid_length);
            var (r_outputs, r_states) = r_cell.Unroll(length, inputs, states.Skip(l_cell.StateInfo().Length).ToArray(), layout, merge_outputs, valid_length);
            ...
            if(!merge_outputs.HasValue)
            {
                ...
                (reversed_r_outputs, _, _) = RNNCell.FormatSequence(null, reversed_r_outputs, layout, merge_outputs.Value);
            }
            ...
            if(merge_outputs.Value)
            {
                if (reversed_r_outputs[0].IsNDArray)
                    reversed_r_outputs = new NDArrayOrSymbol[] { nd.Stack(reversed_r_outputs.ToList().ToNDArrays(), reversed_r_outputs.Length, axis) };
                else
                    reversed_r_outputs = new NDArrayOrSymbol[] { sym.Stack(reversed_r_outputs.ToList().ToSymbols(), reversed_r_outputs.Length, axis) };
            ...
            {
                List<NDArrayOrSymbol> outputs_temp = new List<NDArrayOrSymbol>();
                for(int i=0;i< l_outputs.Length;i++)
                {
                    var l_o = l_outputs[i];
                    var r_o = reversed_r_outputs[i];
                    if (l_o.IsNDArray)
                        outputs_temp.Add(nd.Concat(new NDArray[] { l_o, r_o }, 1));
                    else
