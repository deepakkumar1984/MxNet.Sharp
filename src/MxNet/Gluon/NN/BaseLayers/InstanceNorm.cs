...
namespace MxNet.Gluon.NN
{
    public class InstanceNorm : HybridBlock
    {
                        ...
                        int in_channels= 0, string prefix = null, ParameterDict @params = null) : base(prefix, @params)
        {
            ...
            Gamma = Params.Get("gamma", scale ? OpGradReq.Write : OpGradReq.Null, shape: new Shape((uint)in_channels),init: Initializer.Get(gamma_initializer), allow_deferred_init: true);
            Beta = Params.Get("beta", center ? OpGradReq.Write : OpGradReq.Null, shape: new Shape((uint)in_channels), init: Initializer.Get(beta_initializer), allow_deferred_init: true);
        }
