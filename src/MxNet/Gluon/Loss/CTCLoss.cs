    ...
    public class CTCLoss : Loss
    {
        ...
        public CTCLoss(string layout= "NTC", string label_layout= "NT", float? weight = null, int? batch_axis = 0, string prefix = null, ParameterDict @params = null) : base(weight, batch_axis, prefix, @params)
        {
            ...
            if (label_layout != "NT" && label_layout != "TN")
                throw new ArgumentException($"Only 'NTC' and 'TNC' layouts for label are supported. Got: {label_layout}");
        ...
        private NDArray F(NDArray pred, NDArray label, NDArray pred_lengths= null, NDArray label_lengths= null, NDArray sample_weight= null)
        {
            if (Layout == "NTC")
            ...
            var loss = nd.CTCLoss(pred, label, pred_lengths, label_lengths, 
                                    use_data_lengths: pred_lengths != null, use_label_lengths: label_lengths != null,
                                    CtclossBlankLabel.Last);

            return ApplyWeighting(loss, Weight, sample_weight);
        }

        private Symbol F(Symbol pred, Symbol label, Symbol pred_lengths = null, Symbol label_lengths = null, Symbol sample_weight = null)
        {
            if (Layout == "NTC")
