...
namespace MxNet.GluonCV.Data.Imagenet
{
    public class ImageNet1kAttr
    {
        ...
        {
            Synset = _ILSVRC2012_Attr.Select(x => x.Item1).ToArray();
            Classes = _ILSVRC2012_Attr.Select(x => x.Item2).ToArray();
            ClassesLong = _ILSVRC2012_Attr.Select(x => x.Item3).ToArray();
        }

        public static List<(string, string, string)> _ILSVRC2012_Attr = new List<(string, string, string)> {
